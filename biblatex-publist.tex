...
\date{Version 1.13dev, 2018/11/29}
\maketitle
...
Generally, the sorting of items is done via \bibltx's sorting mechanism, via so called \emph{sorting templates}
(please refer to the \bibltx\ manual for details). 

By default, \bpl\ uses an own template, \jfmacro{ydt},
which sorts hierarchically by year (descending) and title (alphabetically ascending), ignoring author
names. If you use \joption{plauthorhandling=highlight}, however, the default changes to \jfmacro{ydnt}
which sub-sorts by author names (alphabetically ascending) before sub-sorting by title.

In order to diverge from the respective default, you can select a different template via \bibltx's \jfmacro{sorting}
option either globally (by passing \joption{sorting=<template>} to the \bibltx\ options) or locally
(by passing \joption{sorting=<template>} to the options of the \jfcsmacro{printbibliography} command).

Since author name sorting does not make much sense at least in the default configuration,
\bpl\ provides the following additional sorting template that ignores author names, and that is
used by default:
\begin{itemize}
	\item \jmacro{ydt}: Sort by \textbf{y}ear (\textbf{d}escending)
	      and \textbf{t}itle (ascending).
\end{itemize}
For convenience, \bpl\ furthermore provides the following additional sorting templates
(with and without name sorting), which might be particularly useful for sorting talks
(since those usually have more detailed date information):
\begin{itemize}
...
(all ascending).
 ...
 \item \jmacro{dnt}: Sort by full \textbf{d}ate, \textbf{n}ame and \textbf{t}itle
                        (all ascending).
\end{itemize}
To sort your talks in descending order by full date in your CV, thus, use:
\begin{lstlisting}[moretexcs={[1]{printbibliography}}]
