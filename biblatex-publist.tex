...
sec.~\ref{sec:multiauthors} for the case of handling multiple authors.}) once. 
...
\subsection{Highlighting multiple authors}\label{sec:multiauthors}

If the option \joption{plauthorhandling=highlight} has been selected, \bpl\ will consider all \jcsmacro{plauthorname}
specifications you have entered.

Thus it is possible to highlight multiple authors in the publication list (for instance to mark contributions
of a research team):
\begin{lstlisting}[moretexcs={[2]{plauthorname}}]
% All these names will be highlighted
...
entries authored by \emph{any} of the specified persons with \joption{plauthorhandling=highlight}.

Note further that multiple authors are not supported with \joption{plauthorhandling=omit} (since it does not strike me useful).
In this case, only the \emph{last} specified \jcsmacro{plauthorname} is considered.
...
\begin{description}
	...
	\begin{itemize}
		\item Add support for highlighting multiple authors.
...
Marko Budi¨i\'{c}, Clea F. Rees, Yannick Kalff and many other users for
