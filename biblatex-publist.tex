...
Note that if you use this descending enumeration, \bpl\ will have to do a full counting of all printed items first
...
(\jfcsmacro{printbibliography}) that follows. Options that \emph{cannot} be changed within a document that way are
\joption{plnumbering} and \joption{pubstateextra}, as these options employ features that can only be
set once per document. However, please refer to sec.~\ref{sec:numbers} on how to address some frequently requested numbering changes.
...
cycles, but \bpl\ will give you a warning if the numbering is not yet fully resolved.
...
\joption{plnumbering=local} is not used.
With \joption{plnumbering=local}, by contrast, following sections continue to be reset.
...
sections in another one, while using \joption{plnumbering=local} (here both for ascending and descending lists).
...
\jcsmacro{setplnum}, the shifting also persists with \joption{plnumbering=local}.
...
\begin{description}
	...
	\begin{itemize}
		...
		\item Introduce one option (\joption{plnumbering}) that replace the previous two
		      (\joption{plnumbered} + \joption{reversenumbering}).
