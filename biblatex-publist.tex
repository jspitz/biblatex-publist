...
Furthermore, the package employs some newer \LaTeX and \texttt{latex3} features. So a quite
...
In the ascending case only, the macro \jcsmacro*{setbplnum} is useful as a (more flexible) alternative.
...
(provided that ascending numbering is wished for), as does embracing the conference paper in a
\jenviron{bplnumgroup} environment when used together with \joption{plnumbered=reset} (here both for
...
shall take place (e.\,g., \verb|\shiftpblnum{2}| or \verb|\shiftbplnum{-1}|). This might be useful if you
...
\begin{description}
	...
	\begin{itemize}
		...
		\item Add \jenviron{bplnumgroup} environment to temporarily suspend 
