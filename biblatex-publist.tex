...
\date{Version 1.4dev, 2017/02/08}
\maketitle
...
such as the omission or emboldening of the own name from author or editor data. The package requires at least version 3.4 of the \bibltx\ package\footnote{For \bibltx, see
...
\end{lstlisting}
\end{quote}
The \jcsmacro*{plauthorname} macro\footnote{The macro was named \jfcsmacro{omitname} until v.\,1.4 of the \bpl\ package. The old macro still works, but is marked as deprecated.}
(at least with the mandatory \emph{surname} argument) needs to be given once. 
It informs the style file which name it should suppress or embolden in the
author\slash editor list (usually yours). 

With the default settings, the author\slash editor name will be omitted completely for all of your publications
where you are the sole author or editor, as in:
\begin{quote}
...
\end{quote}
If \joption{plauthorhandling=embolden} is used (see next section), the plauthor
will be emboldened instead, as in:
\begin{quote}
...
\item [{\joption{plauthorhandling{[}=omit|embolden{]}}}] default: \emph{omit}.

By default, the publist author (as defined with \jmacro{plauthor}) is omitted from
the author or editor list. If you use the option \joption{plauthorhandling=embolden},
it is set in bold face instead.
