...
The macro \jcsmacro*{setbplnum} allows you to set the first label number of the next bibliography
section that follows to a specific value, which is passed as argument. All following numbers (across bibliography
sections) ascend from the new value. Note that this command does not make sense, and thus is disabled,
with \joption{reversenumbering=true}.
...
\begin{description}
	...
	\begin{itemize}
		...
		\item Add new macro\jcsmacro{setbplnum} (see sec.~\ref{sec:auxmacros}).
