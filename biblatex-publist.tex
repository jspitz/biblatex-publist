...
\date{Version 1.17dev, 2020/06/27}
\maketitle
...
the URL field, the ISBN field or the ISSN field (checked in this order) is defined for the given entry.

With URL and DOI, direct links are used. With ISBN or ISSN, a link to a search provider is used instead (\textsf{worldcat}
by default). The search provider can be customized by redefining the following macros:
\begin{lstlisting}[moretexcs={[2]{bplbookurlisbn,bplbookurlissn}}]
\newcommand*\bplbookurlisbn[1]{%
    https://www.worldcat.org/search?qt=worldcat_org_all&q=#1}
\newcommand*\bplbookurlissn[1]{%
    https://www.worldcat.org/search?qt=worldcat_org_all&q=#1}
\end{lstlisting}
%
Note that the output of URLs, DOIs, ISBNs and ISSNs is not affected by the \joption{linktitle} options.
To control them, use the \joption{url}, \joption{doi} and \joption{isbn} biblatex options. 
