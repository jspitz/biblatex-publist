...
\newcommand*\@marginmacro[2][]{%
	\marginpar{\IfBlankTF{#1}{}{\hspace*{#1em}}\small\texttt{#2}}%
}
...
\newcommand*\@@jenviron[1]{\marginpar{\small\texttt{#1}}%
...
\date{Version 2.0dev, 2022/10/19}
\maketitle
...
thereby account for my own needs and requests that reached me.
...
number of entries back to 1. This also works with sectional numbering (\joption{plnumbered=reset});
...
\end{description}
%
These options should be passed directly to \bibltx\ via \jfcsmacro{usepackage} when loading \bibltx, and they apply for the whole
document.

For most options, it is possible to change them (repeatedly) within the document. To this end, the macro \jcsmacro*[-3]{ExecutePublistOptions}
is provided. It takes a comma-separated list of options as its mandatory argument and applies as of the next bibliography
(\jfcsmacro{printbibliography}) that follows. Options that \emph{cannot} be changed within a document that way include:
\joption{plnumbered}, \joption{reversenumbering}, and \joption{pubstateextra}, as these options employ features that can only be
set once for a document.
...
\begin{itemize}
	...
	\item With \joption{plauthorhandling}, \bpl\ uses different sorting schemes for \joption{omit} (\texttt{ydt})
	      ...
	      So changing this option within a document might reset your own sorting. You can change that back via
	      \jfcsmacro{newrefcontext[sorting=<sorting>]}.
\end{itemize}
...
of publications a scholar has is, next to weighted references to them, social capital. The numbering of items in
publication lists is thus, if you will, a sort of account balance.
...
\begin{enumerate}\setlength{\itemsep}{0pt}
	...
	\item Local ascending numbers (that are reset to 1 in each section) [\joption{plnumbered=reset}]
...
should be numbered continuously within their category. So \joption{plnumbered=reset} alone does not produce
the desired result.

For this case, \bpl\ provides two alternative and slightly different features: First, an environment
\jenviron*{plnumgroup}.
All sections you embrace by this in the context of \joption{plnumbered=reset} are numbered continuously,
...
conference papers, together with \joption{plnumbered=true} (the default), will provide the desired result
...
sections in another one, while using \joption{plnumbered=reset} (here both for ascending and descending lists).

The macro \jcsmacro*{shiftplnum}, by contrast, provides a way to adjust the subsequent numbering in a
\emph{relative} way.
...
\jcsmacro{setplnum}, the shifting also persists with \joption{plnumbered=reset}.
...
\begin{description}
	...
	\begin{itemize}
		...
		\item Add \jenviron{plnumgroup} environment to temporarily suspend 
		      \joption{plnumbered=reset} (see sec.~\ref{sec:numbers}).
		...
		\item Fix macro \jcsmacro{shiftplnum} with \joption{reversenumbering=false}.
	\end{itemize}
