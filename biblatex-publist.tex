...
want to reveal your identity yet), or you want to separately list exactly these publications.
To support this, \bpl\ features some \joption{bibchecks}
...
\begin{itemize}
	...
	\item \joption{onlysubmitted}: reversely shows only entries with pubstate \joption{submitted}.
	\item \joption{noprepared}: omits all entries with pubstate \joption{inpreparation}.
	\item \joption{onlyprepared}: reversely shows only entries with pubstate \joption{inpreparation}.
	...
	\item \joption{nopublished}: reversely omits all published entries (i.e., entries that do not have
	      a pubstate) as well as \joption{prepublished} entries.
    ...
    \item \joption{noaccepted}: reversely omits all published entries (i.e., entries that do not have
	      a pubstate) as well as \joption{prepublished} and \joption{forthcoming}
	      (and thus accepted) entries.
\end{itemize}
%
...
\end{lstlisting}
%
but of course, some are also mutually exclusive (logically).
...
In addition to the default \joption{ydt} template, \bpl\ provides the following sorting templates
that also do not account for author names in sorting:
\begin{itemize}
...
\begin{description}
	\item [{V. 2.16 (forthcoming):}]~
	\begin{itemize}
		\item Add Greek localization (provided by Nikos Platis).
		\item Fix bug with et al. after publist author.
		\item Fix \texttt{maxnames} with \texttt{plauthor=omit}.
	\end{itemize} 
	\item [{V. 2.15 (2025/09/10):}]~
	\begin{itemize}
		\item Add bibchecks \texttt{onlysubmitted}, \texttt{onlyprepared}, \texttt{nopublished}
		      and \texttt{noaccepted} (see sec.~\ref{sec:filterbc}).
		\item Remove sorting templates \texttt{dnt} and \texttt{ddnt} as these are provided
		      by \bibltx\ itself.
	\end{itemize} 
...
Nikos Platis for providing Greek localization,
