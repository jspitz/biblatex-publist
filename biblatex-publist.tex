...
\date{Version 1.28dev, 2022/10/15}
\maketitle
...
number of entries back to 1. This also works with descending order (\joption{plnumbered=reset});
in this case, the numbers descend to 1 in each section. Obviously the option is nonfunctional with
\joption{plnumbered=false}.

Note that if you use this option, \bpl\ will have to do a full counting of all printed items first in order
to calculate the numbering properly (considering any filtering you might have done).
...
\subsection{Tweaking the numbers}\label{sec:numbers}

Among the peculiarities of publication lists is the greater variety of numbering conventions, as compared
with bibliographies in academic works. If a numeric citation style is used in the latter, the items are
as a rule numbered in ascending order, and each entry has a unique number. The main function of the
number here is to provide fast access from a reference to the referred entry.

In publication lists, this is only a secondary function of the number. The main function here is, in fact,
metrical. In our increasingly metricalized academic cultures, and against the backdrop of the
\enquote{publish or perish} imperative scholars find themselves confronted with, the number of publications
a scholar has is, next to weighted references to them, social capital. The numbering of items in publication lists
is thus, if you will, a sort of account balance. Since publication formats (e.\,g., journal papers vs. chapters)
are usually valued differently, many people prefer to have a more differentiated balance in the sense that the
publication lists swiftly tells you how many papers, monographs, chapters, etc., a person has published.
Hence those people prefer to number their publications by section.
...
The \bpl\ package accounts for these needs and provides, via the options \joption{plnumbered} and
\joption{reversenumbering} five different numbering variants:
\begin{enumerate}\setlength{\itemsep}{0pt}
	\item Global ascending numbers (that continues across sections) [\emph{default}]
	\item Global descending numbers [\joption{reversenumbering=true}]
	\item Local ascending numbers (that is reset in each section) [\joption{plnumbered=reset}]
	\item Local descending numbers [\joption{reversenumbering=true, plnumbered=reset}]
	\item No numbers at all [\joption{plnumbered=false}]
\end{enumerate} 
%
Particularly reverse numbering is, technically, a challenge; \bibltx, for reasons, does not provide this
...
only knows this number after all entries have been printed. Yet since it only provides for ascending numbers,
\bibltx\ does not (need to) store this knowledge, and thus, it is not readily available to \bpl.

Thus, with \joption{reversenumbering=true}, \bpl\ will first count all printed entries, globally and per section,
write the results in an auxiliary file (\emph{<filename>.bpx}), and then do at least one additional pass in order
to fix the numbering. This requires additional \texttt{latex} cycles, but \bpl\ will give you a warning if the
numbering is not yet fully resolved.
...
same document, but separately numbered. Both publications and talks might be divided into different sections
which, however, should be numbered continuously. So \joption{plnumbered=reset} is not an option here.

In this case, the macro \jcsmacro*{setbplnum} is useful. It allows you to set the first label number of the
next bibliography section that follows to a specific value. All following numbers ascend or descend
from this new value.
The adjustment applies to all following bibliography sections with continuous numbering, i.\,e., if
\joption{plnumbered=reset} is not used.
With \joption{plnumbered=reset}, by contrast, following sections continue to be reset.
In all cases, the macro can be used repeatedly for re-adjustments in forthcoming sections.

So in the use case mentioned above, setting \verb|\setbplnum{1}| before the first section that lists
conference papers will provide the desired result (provided that ascending numbering is wished for).

The macro \jcsmacro*{shiftbplnum}, by contrast, provides a way to adjust the subsequent numbering in a
\emph{relative} way.
It takes a positive or negative integer value that determines how much and in which direction the adjustment
shall take place (e.\,g., \verb|\shiftpblnum{2}| or \verb|\shiftbplnum{-1}|). Again, this macro can be used
repeatedly  and applies to all subsequent items as of the next section that follows. As opposed to
\jcsmacro{setbplnum}, the shifting also persists with \joption{plnumbered=reset}.
...
\begin{description}
	...
	\item [{V. 1.28 (forthcoming):}]~
	\begin{itemize}
		\item Properly support \joption{reversenumbering} even if filtering is done.
		\item Add new macro\jcsmacro{setbplnum} (see sec.~\ref{sec:numbers}).
		\item Fix macro \jcsmacro{shiftbplnum} with \joption{reversenumbering=false}.
	\end{itemize}
