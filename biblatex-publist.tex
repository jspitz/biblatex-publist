...
Even though publication lists typically do not cite, there might be cases where you need to refer to specific entries within
it. Especially with sectioned lists, it is arguably most advisable to refer to the items by their number, as otherwise
your readers will have a hard time to find the item you are referring to. Of course, if you do not use global numbering,
you should also specify the section the item is in (as in: ``see [13] in section~4''.).

Since \bpl\ plays all sorts of tricks with the numbering (see sec.~\ref{sec:numbers}), the citation needs to take these
...
Note, however, that with refsections (see sec.~\ref{sec:example}) \jcsmacro{citeitem} is restricted to the scope
of the current refsection because the citations of a given refsection are unknown outside of its scope.

If you want to refer to an item number from outside a refsection, you can use \jcsmacro*{citesecitem}\jmacro{\{<key>\}}.
...
\jcsmacro*{citesecitem*}\jmacro{\{<key>\}} which will append a reference to the section number. By default, the form of
this reference is ``(sec.~<no.>)'', with ``sec.'' being localized. You can change this by redefining the macro:
\begin{lstlisting}[moretexcs={[2]{mkbibparens,secitemsecref,bibstring,addnbspace}}]
\newcommand*\secitemsecref[1]{%
	\mkbibparens{\bibstring{section}\addnbspace#1}%
}
\end{lstlisting}

Note that due to the design of refsections, no hyperlinks will be employed with
\jcsmacro{citesecitem}. If you need them, consider refsegments rather than refsections and the \jcsmacro{citeitem} command.

Of course, both \jcsmacro{citeitem} and \jcsmacro{citesecitem} presuppose that you do not disable numbering.
